...
1. **Turn scaffold**: at every wrap-up, milestone gate, or fork in the road, append to your reply (native Markdown, never code blocks). A wrap-up looks like this (compact one-line form, semantically identical to the multi-line list in protocol §3; sample content — do not reuse):
...
2. **Capability numbers**: the "Capability menu" below is the single registry — one row per new capability; numbers are never reused.
   Say "menu" to see the table; say a code (e.g. E1) to run it. When registry rows would push this file past the 40-line red line, externalize them to a registry file — same four-column table, group rows occupy a full row, e.g. `| Group D | writing | writing workflows registered there | docs/capabilities.md |` — and keep only group rows and pointers here.
